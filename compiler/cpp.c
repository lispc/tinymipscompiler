...
int get_pos(char* s)
{
  ...
  ((1==0) ? (void) (0) : __assert_fail ("1==0", "codegen.c", 65, __PRETTY_FUNCTION__));
}
...
int ex(oprNodeType *p) {
  ...
  case '=':
    ...
    pos = get_pos(name);
    if(p->nops==3){
      ((pos!=-1) ? (void) (0) : __assert_fail ("pos!=-1", "codegen.c", 188, __PRETTY_FUNCTION__));
      ex(p->op[2]);
      ex(p->op[1]);
      printf("\tpush\t%d\n",pos);
      printf("\tadd\n");
      ...
      if(p->op[1]->type==typeArr){
        ((pos==-1) ? (void) (0) : __assert_fail ("pos==-1", "codegen.c", 198, __PRETTY_FUNCTION__));
